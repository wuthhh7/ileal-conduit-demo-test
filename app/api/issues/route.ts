...
export async function GET(request: Request) {
  ...
  const issues = await getD1().prepare(`select i.id, i.patient_id as "patientId", p.line_user_id as "lineUserId", p.display_name as "displayName", p.avatar_url as "avatarUrl",
