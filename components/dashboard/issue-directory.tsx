...
import { CheckCircle2, Clock3, MessageCircleReply, Search, Send } from 'lucide-react';
...
export function IssueDirectory({ view }: { view: 'unanswered' | 'answered' }) {
  ...
  const visibleIssues = useMemo(() => {
    ...
    return issues.filter((issue) => issue.status === view && (!normalized || [issue.displayName, issue.subject, issue.category, issue.detail].join(' ').toLocaleLowerCase('th').includes(normalized)));
  }, [issues, query, view]);
  ...
  async function reply(event: SyntheticEvent<HTMLFormElement>, issue: Issue) {
    ...
    setError('');
    const response = await fetch(`/api/issues/${encodeURIComponent(issue.id)}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ reply: replyText }) });
    const body = await response.json().catch(() => ({}));
    if (response.ok) { setActiveReply(null); await load(); }
    else setError(body.error || 'ส่งคำตอบไม่สำเร็จ กรุณาลองใหม่');
    setSendingId(null);
  }

  if (needsLogin) return <DashboardLogin onSuccess={load}/>;
  const unanswered = view === 'unanswered';
  return <DashboardShell title={unanswered ? 'เรื่องที่ยังไม่ตอบกลับ' : 'เรื่องที่ตอบกลับแล้ว'} description={unanswered ? 'ปัญหาที่ผู้ป่วยส่งจาก LINE และกำลังรอคำตอบจากพยาบาล' : 'ประวัติเรื่องที่พยาบาลตอบกลับทาง LINE เรียบร้อยแล้ว'} onRefresh={load}>
    {error && <div className={styles.error}>{error}</div>}
    <div className={styles.issueToolbar}><label><Search/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ค้นหาชื่อผู้ป่วย หัวข้อ หรือรายละเอียด..."/></label><span>{visibleIssues.length} เรื่อง</span></div>
